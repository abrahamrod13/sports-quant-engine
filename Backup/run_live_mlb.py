...
print("=" * 60)
print("⚾ MLB QUANT ENGINE V8 - ANÁLISIS COMPLETO HOY")
print("=" * 60)
print(f"📅 {datetime.now().strftime('%Y-%m-%d %H:%M')}")
print(f"🎯 MLB: Edge>0.02 | Prob≥0.52 | Vol≤0.68 | Context≥0.38")
print("=" * 60)
...
if len(mlb_games) > 0:
    ...
    # DEDUPLICAR: solo un registro por juego
    ...
    for _, row in mlb_games.iterrows():
        ...
        print(f"      💰 ML: {home_team} ({h2h_home}) | {away_team} ({h2h_away})")
        print(f"      💰 Spread: {home_team} {spread_point} ({spread_home}) | {away_team} +{spread_point} ({spread_away})")
        print(f"      💰 Total: O/U {total_point} (O:{total_over} / U:{total_under})")
        ...
        print(f"      🔥 {home_p_name}: {home_p_data.get('era','?')} ERA | {home_p_data.get('whip','?')} WHIP | {home_p_data.get('k9','?')} K9")
        print(f"      🔥 {away_p_name}: {away_p_data.get('era','?')} ERA | {away_p_data.get('whip','?')} WHIP | {away_p_data.get('k9','?')} K9")
        ...
        # ============================================
        # ANALIZAR OTROS BET TYPES (SIEMPRE)
        # ============================================
        
        # Determinar favorito según el modelo
        ...
        if result['probability'] >= 0.55:
            ...
            print(f"      │  Runline {model_fav} -1.5: {rl_prob:.1%} | Edge: {rl_edge:+.1%} {'✅' if rl_edge > 0.02 else '🟡'}")
        ...
        # Pitchers malos = over, pitchers buenos = under
        if home_eff_era > 5.5 and away_eff_era > 5.5:
            total_prob = 0.62  # Over probable
        elif home_eff_era < 3.0 and away_eff_era < 3.0:
            total_prob = 0.38  # Under probable
        ...
        print(f"      │  Over {total_point}: {total_prob:.1%} | Edge: {over_edge:+.1%} {'✅' if over_edge > 0.02 else '🟡'}")
        print(f"      │  Under {total_point}: {under_prob:.1%} | Edge: {under_edge:+.1%} {'✅' if under_edge > 0.02 else '🟡'}")
        ...
        print(f"      │  {home_team} Team Over 3.5: {home_over_prob:.1%} {'✅' if home_over_prob > 0.55 else '🟡'}")
        print(f"      │  {away_team} Team Over 3.5: {away_over_prob:.1%} {'✅' if away_over_prob > 0.55 else '🟡'}")
        
        # 4. FIRST 5 INNINGS
        f5_prob = result['probability']
        f5_edge = f5_prob - 0.45
        print(f"      │  First 5 ML {model_fav}: {f5_prob:.1%} | Edge: {f5_edge:+.1%} {'✅' if f5_edge > 0.02 else '🟡'}")
...
print("=" * 60)
print("✅ Análisis MLB completado - V8 |", datetime.now().strftime('%H:%M'))