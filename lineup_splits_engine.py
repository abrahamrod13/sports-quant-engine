...
def get_team_splits(team_name, split='vsLeft'):
    ...
    try:
        ...
        return {'avg': stat.get('avg','.250'), 'ops': stat.get('ops','.720'), 'hr': stat.get('homeRuns',0), 'so': stat.get('strikeOuts',0)} if stat else None
    except: return None
...
def lineup_split_advantage(home_team, away_team, away_pitcher_throws='R'):
    ...
    ops = float(home_splits['ops'])
    if ops > 0.750: return 0.02
    elif ops < 0.680: return -0.02
    return 0