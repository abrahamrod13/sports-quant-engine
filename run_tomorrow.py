...
from live_fetcher import (
    get_tomorrow_nba_games, get_tomorrow_soccer_matches,
    get_tomorrow_odds
)
...
print("=" * 60)
print("🏀⚾⚽ SPORTS QUANT ENGINE V7 - PREDICCIONES MAÑANA")
print("=" * 60)
...
if len(nba_games) > 0:
    nba_odds = get_tomorrow_odds('basketball_nba')
    if len(nba_odds) > 0:
...
# ============================================
# MLB - V5 + V6 + V7 + FANDUEL
# ============================================
print("⚾ MLB MAÑANA...")
mlb_games = get_tomorrow_mlb_games()

if len(mlb_games) > 0:
    mlb_odds = get_tomorrow_odds('baseball_mlb')
    if len(mlb_odds) > 0:
...
# ============================================
print("⚽ SOCCER MAÑANA...")
soccer_matches = get_tomorrow_soccer_matches()

if len(soccer_matches) > 0:
    soccer_odds = get_tomorrow_odds('soccer_epl')
    for league_odds in ['soccer_spain_la_liga', 'soccer_italy_serie_a', 'soccer_uefa_champs_league']:
        extra_odds = get_tomorrow_odds(league_odds)
        if len(extra_odds) > 0:
...
# ============================================
print("=" * 60)
print(f"📊 PREDICCIONES PARA MAÑANA ({tomorrow.strftime('%d/%m/%Y')})")
print("=" * 60)

if len(all_setups) == 0:
    print("\n🔴 0 SETUPS A+ para mañana")
    print('🧠 "No trade" anticipado. Preserva capital.')
else:
    ...
    for i, s in enumerate(all_setups[:5], 1):
        ...
        league_info = f" ({s.get('league', '')})" if 'SOCCER' in s.get('sport', '') else ''
        print(f"  {i}. {s['sport']}{edge_quality}{v6_edge}{v7_edge}{league_info} | {s['home_team']} vs {s['away_team']}")
        print(f"     👉 GANA: {s['pick']}")
...
print("=" * 60)
print("✅ Predicciones completadas - V7 Market Exploiter")